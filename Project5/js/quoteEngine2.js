...
function wordLookUp(word, task, number){
  ...
  params = 'word=' + word + '&task=' + task + '&max=' + number.toString();
  if (number <1 || number == undefined){number = 10;}
  if (task === "ml="|| task === "sp="|| task === "sl="|| task === "rel_trg="|| task === "rel_jja="|| task === "rel_jjb="||
    ...
    xhr.onload = function(){
      if(xhr.readyState == 4 && xhr.status == 200){
        if (xhr.responseText.charAt(0) === "[" && xhr.responseText.charAt(xhr.responseText.length-1) === "]"){
          handleResponse(xhr.responseText);
        }else{
          errorFlag = true;
          rqOutputWindow.innerHTML = "<h1>Word ERROR! Word must contain only letters & no spaces.</h1>";
        };
        
      }else{
      //output an error to the window if the request fails
      errorFlag = true;
      rqOutputWindow.innerHTML = "<h1>API ERROR! Please try again.</h1>";
    }
  };
  //set a timeout handler for requests, we don't want to wait forever
  xhr.ontimeout = function(){
    errorFlag = true;
    rqOutputWindow.innerHTML = "<h1>API timeout. Please try again.</h1>";
  };
  
  xhr.send(params);
}else{
  errorFlag = true;
  rqOutputWindow.innerHTML = "<h1>parameter ERROR! Please check word.</h1>";
};
}
...
function handleResponse(response) {
  ...
  if (errorFlag === false){
   if (quoteStage === 0){
         //Try to correct spelling of seedword, if no corrections in JSON, output an error.
         if (mlJSON.length === 0){
          errorFlag = true;
          rqOutputWindow.innerHTML = "<h1>Spellcheck failed! Check spelling and remove any spaces.</h1>";
        }
        if (mlJSON.length >= 1 && spellCheckOK === false){
          spellCheckOK = true;
          seedWord = mlJSON[0].word;
          //get a broader word from the orignial seedword
          wordLookUp(mlJSON[0].word, searchParam.searchType[quoteStage], 100);
          ...
          wordLookUp("wisdom", searchParam.searchType[quoteStage], 100);//try a basic noun as a last resort
        }
      }
    }
      //Get the rest of the words
      if (quoteStage >= 1 && quoteStage < quoteWords.length && i === false){
      ...
      if (quoteStage < quoteWords.length){
        ...
        //special case for verbs and past participles, need a massive list to find related verbs from DataMuse API
        else if (quoteStage === 2 || quoteStage === 3 || quoteStage === (quoteWords.length-2) || quoteStage === (quoteWords.length-1)){
  ...
  if (errorFlag === false){
    ...
    if (quoteStage === quoteWords.length){constructQuote();rqOutputWindow.innerHTML = "<p>RoboQuote about: <em>" + seedWord + "</em></p><p>" + quote + "</p>";}
  }
}
}
