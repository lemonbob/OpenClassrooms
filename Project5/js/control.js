...
var controlKnobs = document.getElementsByClassName("knob-image");
		var knobWrappers = document.getElementsByClassName("control-knob");
	...
	//add Event Listeners
	/*
	for(i = 0;i<controlKnobs.length;i++){
		addClosureEvent(controlKnobs,i,["mousedown", "touchstart"]);
	}
	*/
	for(i = 0;i<knobWrappers.length;i++){
		addClosureEvent(knobWrappers,i,"mousedown");
		addClosureEvent(knobWrappers,i,"touchstart");
	}

	
	button.addEventListener("touchstart",function(){button.style.transform = "scale(1,1)";button.style.boxShadow = "0px 0px 20px #000";});
	button.addEventListener("touchend",function(e){e.preventDefault();button.style.transform = "scale(1.1,1.1)";button.style.boxShadow = "5px 5px 20px #000";quoteButtonClick();});
	...
	document.addEventListener("touchend",function(e){e.preventDefault();resetPointerStatus();});
	...
	function addClosureEvent(element,item,event){
		...
		el[num].addEventListener(event,function(e){setMousePosition(e);controlNum = num;});
	}
